...
function getEdgeGrowthStart(trajectory: Trajectory) {
  return Math.min(
...
function getSquareScale(progress: number, trajectory: Trajectory) {
  ...
  const edgeGrowthStart = getEdgeGrowthStart(trajectory);
...
function getSquareWidthScale(progress: number, trajectory: Trajectory) {
  const widthChangeStart = getEdgeGrowthStart(trajectory);

  if (progress <= widthChangeStart) {
    return 1;
  }
  ...
  const sideProgress = THREE.MathUtils.clamp(
    THREE.MathUtils.inverseLerp(
      widthChangeStart,
      trajectory.sideProgress,
      progress,
    ),
    0,
    1,
  );
  const widthAtSide = THREE.MathUtils.clamp(SQUARE_WIDTH_AT_SIDE, 0, 1);

  return THREE.MathUtils.lerp(1, widthAtSide, sideProgress);
}
...
export function getSpeedMultiplier(progress: number, trajectory: Trajectory) {
...
export function placeSquareOnPath(
  ...
  mesh.lookAt(0, 0, 0);
  const scale = getSquareScale(progress, trajectory) * reveal;
  mesh.scale.set(
    scale * getSquareWidthScale(progress, trajectory),
    scale,
    scale,
  );
